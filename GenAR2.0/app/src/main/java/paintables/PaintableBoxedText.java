...
/**
 * Draws text on the canvas surrounded by a box
 */
public class PaintableBoxedText extends PaintableObject {
	...
	private void prepTxt(String txtInit, float fontSizeInit, float maxWidth) {
		...
		float areaWidth = maxWidth - pad;
		lineHeight = getTextAsc() + getTextDesc();

		if (lineList==null) lineList = new ArrayList<String>();
		else lineList.clear();

        int distPos = txt.indexOf('(');
		String dist = txt.substring(distPos, txt.length());

        txt = txt.replace(dist, "");
		...
		float areaHeight = lineHeight * lines.length;
